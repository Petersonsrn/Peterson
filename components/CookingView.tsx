...
export const CookingView: React.FC<CookingViewProps> = ({ recipe, onClose }) => {
  ...
  // Refs para gerenciamento de foco do teclado
  ...
  // Memoize a lista de elementos focáveis para a "armadilha de foco".
  // Esta lista recalcula apenas quando o passo muda, o que pode desabilitar/habilitar os botões de navegação.
  ...
  // Efeito para definir o foco inicial no botão de fechar ao abrir o modal
  useEffect(() => {
    closeButtonRef.current?.focus();
  }, []);

  // Efeito para gerenciar toda a navegação por teclado (Setas, Esc, Tab)
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Tab') {
        if (focusableElements.length < 2) { // Trava o foco apenas se houver mais de um elemento
          e.preventDefault();
          return;
        }
